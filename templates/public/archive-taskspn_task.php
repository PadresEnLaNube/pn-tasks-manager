 ...
 */
	...
	if (class_exists('Polylang')) {
		$taskspn_tasks = get_posts(['numberposts' => -1, 'fields' => 'ids', 'post_type' => 'taskspn_task', 'lang' => pll_current_language(), 'post_status' => ['publish'], 'order' => 'DESC', ]);
	} else {
		$taskspn_tasks = get_posts(['numberposts' => -1, 'fields' => 'ids', 'post_type' => 'taskspn_task', 'post_status' => ['publish'], 'order' => 'DESC', ]);
	}
?>
	<body <?php body_class(); ?>>
		<div class="taskspn-wrapper taskspn-task-wrapper">
		  <h1 class="taskspn-p-20"><?php esc_html_e('Base CPT', 'taskspn'); ?></h1>
			...
			<div class="taskspn-display-table taskspn-width-100-percent taskspn-mt-50 taskspn-mb-50">
				<?php if (!empty($taskspn_tasks)): ?>
			  	<?php foreach ($taskspn_tasks as $taskspn_task_id): ?>
						<div class="taskspn-display-inline-table taskspn-width-33-percent taskspn-tablet-display-block taskspn-tablet-width-100-percent taskspn-p-20 taskspn-text-align-center taskspn-vertical-align-top">
							<div class="taskspn-mb-30">
								<a href="<?php echo esc_url(get_permalink($taskspn_task_id)); ?>">
									<?php if (has_post_thumbnail($taskspn_task_id)): ?>
								    <?php echo get_the_post_thumbnail($taskspn_task_id, 'full', ['class' => 'taskspn-border-radius-20 taskspn-width-100-percent']); ?>
								  <?php else: ?>
							...
							<a href="<?php echo esc_url(get_permalink($taskspn_task_id)); ?>"><h4 class="taskspn-color-main-hover taskspn-mb-20"><?php echo esc_html(get_the_title($taskspn_task_id)); ?></h4></a>

							<?php if (current_user_can('administrator') || current_user_can('taskspn_role_manager')): ?>
				  			<a href="<?php echo esc_url(admin_url('post.php?post=' . $taskspn_task_id . '&action=edit')); ?>"><i class="material-icons-outlined taskspn-font-size-30 taskspn-vertical-align-middle taskspn-mr-10 taskspn-color-main-0">edit</i> <?php esc_html_e('Edit task', 'taskspn'); ?></a>
				  		<?php endif ?>
						</div>
			  	<?php endforeach ?>
				<?php endif ?>
